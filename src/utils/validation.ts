...
export function validateConversionResult(
  ...
  if (expected.tithi) {
    ...
    if (expected.tithi.adjustedNumber && result.tithi.adjustedNumber !== expected.tithi.adjustedNumber) {
      return false;
    }
  ...
  if (expected.month) {
    ...
    if (expected.month.isSkipped !== undefined && result.month.isSkipped !== expected.month.isSkipped) {
      return false;
    }
  }
