import { NSDate, NSMonth, NSTithi, NSAstronomical } from "../constants/types";
import {
...
export function convertToNSDate(date: Date): NSDate {
  ...
  const tithi = convertToTithi(year, month, day);
...
export function convertToNSMonth(
  year: number,
  month: number,
  day: number
): NSMonth {
  ...
  const isLeap = checkForAnala(year, month, day);
  // Check for Nhalā (skipped month)
  const isSkipped = checkForNhala(year, month, day);
...
export function convertToTithi(
  year: number,
  month: number,
  day: number
): NSTithi {
...
function checkForAnala(year: number, month: number, day: number): boolean {
...
function checkForNhala(year: number, month: number, day: number): boolean {
